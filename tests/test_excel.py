...
def test_xl_1_to_1(tmp_path):
    th.f_1_to_1([push, pull], tmp_path)


def test_xl_2_to_2(tmp_path):
    th.f_2_to_2([push, pull], tmp_path)


def test_xl_2_to_1(tmp_path):
    th.f_2_to_1_together([push, pull], tmp_path)


def test_xl_skiprows(tmp_path):
    df0 = pd.DataFrame({"a": [1, 2, 3]})

    push_old(tmp_path, {"df": df0}, {"to_excel": {"startrow": 2}})
...
def test_append_fixed(tmp_path):
    th.append_fixed([push, pull], tmp_path=tmp_path)
...
def test_truncate(tmp_path):
    th.truncate2(push, pull, tmp_path)
...
def test_truncate_under(tmp_path):
    ...
    push_old(tmp_path, {"df": df0b}, {"if_exists": "truncate"})
